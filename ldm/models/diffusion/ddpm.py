...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c, rel_class_id):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c = self.cond_stage_model.encode(c, rel_class_id, embedding_manager=self.embedding_manager)
    ...
    def shared_step(self, batch, **kwargs):
        ...
        loss = self(x, c, batch['rel_class_id'])
        return loss

    def forward(self, x, c, rel_class_id, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
            assert c is not None
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c, rel_class_id)

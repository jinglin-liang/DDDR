...
class GlobalSynthesizer(ABC):
    def __init__(self, teacher, student, generator, nz, num_classes, img_size,
        ...
        self.normalizer = Normalizer(**dict(hyperparams[args["dataset"]]["data_normalize"]))
...
class TARGET(BaseLearner):
    def __init__(self, args):
        ...
        self.syn_image_dir = os.path.join(args["save_dir"], "syn_imgs")
        self.args = args
        self.normalizer = Normalizer(**hyperparams[args["dataset"]]["data_normalize"])
        os.makedirs(self.syn_image_dir, exist_ok=True)
    ...
    def kd_train(self, student, teacher, criterion, optimizer):
        ...
        for _ in range(hyperparams[self.args["dataset"]]["kd_steps"]):
    ...
    def data_generation(self):
        ...
        tmp_dir = os.path.join(self.syn_image_dir, "task_{}".format(self._cur_task))
        ...
        synthesizer = GlobalSynthesizer(copy.deepcopy(self._network), student, generator,
                    ...
                    save_dir=tmp_dir,
                    transform=hyperparams[self.args["dataset"]]["train_transform"], 
                    normalizer=self.normalizer,
                    synthesis_batch_size=hyperparams[self.args["dataset"]]["synthesis_batch_size"], 
                    sample_batch_size=hyperparams[self.args["dataset"]]["sample_batch_size"],
                    iterations=hyperparams[self.args["dataset"]]["g_steps"], 
                    warmup=hyperparams[self.args["dataset"]]["warmup"], 
                    lr_g=hyperparams[self.args["dataset"]]["lr_g"], 
                    lr_z=hyperparams[self.args["dataset"]]["lr_z"],
                    adv=hyperparams[self.args["dataset"]]["adv"], 
                    bn=hyperparams[self.args["dataset"]]["bn"], 
                    oh=hyperparams[self.args["dataset"]]["oh"],
                    reset_l0=hyperparams[self.args["dataset"]]["reset_l0"], 
                    reset_bn=hyperparams[self.args["dataset"]]["reset_bn"],
                    bn_mmt=hyperparams[self.args["dataset"]]["bn_mmt"], 
                    is_maml=hyperparams[self.args["dataset"]]["is_maml"], 
                    args=self.args)
        
        criterion = KLDiv(T=hyperparams[self.args["dataset"]]["T"])
        ...
        for it in range(hyperparams[self.args["dataset"]]["syn_round"]):
            synthesizer.synthesize() # generate synthetic data
            if it >= hyperparams[self.args["dataset"]]["warmup"]:
                ...
                print("Task {}, Data Generation, Epoch {}/{} =>  Student test_acc: {:.2f}".format(
                    self._cur_task, it + 1, hyperparams[self.args["dataset"]]["syn_round"], test_acc,))
                scheduler.step()
    ...
    def get_syn_data_loader(self):
        ...
        data_dir = os.path.join(self.syn_image_dir, "task_{}".format(self._cur_task-1))
        print("iters{}, syn_bs:{}, data_dir: {}".format(iters, syn_bs, data_dir))

        syn_dataset = UnlabeledImageDataset(data_dir, transform=hyperparams[self.args["dataset"]]["train_transform"], nums=self.nums)
    ...
    def get_all_syn_data(self):
        data_dir = os.path.join(self.syn_image_dir, "task_{}".format(self._cur_task))
        syn_dataset = UnlabeledImageDataset(data_dir, transform=hyperparams[self.args["dataset"]]["train_transform"], nums=self.nums)
        loader = DataLoader(
            syn_dataset, batch_size=hyperparams[self.args["dataset"]]["sample_batch_size"], shuffle=True,
    ...
    def _local_finetune(self, teacher, model, train_data_loader):
        # global print_flag
        model.train()
        teacher.eval()
        optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9, weight_decay=5e-4)
        for _ in range(self.args["local_ep"]):
            ...
            for _, ((_, images, labels), syn_input) in iter_loader:
                ...
                loss = loss_ce + self.args["w_kd"]*loss_kd 
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        return model.state_dict()
    ...
    def _fl_train(self):
        ...
        for _, com in enumerate(prog_bar):
            ...
            for idx in idxs_users:
                if self._cur_task == 0:
                    w = self._local_update(copy.deepcopy(self._network), self.local_train_loaders[idx])
                else:
                    w = self._local_finetune(self._old_network, copy.deepcopy(self._network), 
                                             self.local_train_loaders[idx])
    ...
    def init_dataloader(self, data_manager):
        super().init_dataloader(data_manager)
        if self._cur_task != 0: 
